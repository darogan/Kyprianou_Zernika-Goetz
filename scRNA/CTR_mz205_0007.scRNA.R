...
plt.ftr   <- FeaturePlot(matrix.su, cols=c("blue","lightgrey","red"), ncol=1, features = c("Pou5f1","Bmp4","Amn"), reduction="umap")
...
plt.dims               <- plot_grid(plt.dim, plt.dim.lab, ncol=1)
plt.dim.red            <- plot_grid(plt.dims, plt.ftr, ncol=2)
...
png(paste0(Project, "_scUMAPs", ".png"), units="cm", width=10, height=10, res=180)
par(bg=NA)
plt.dim.red 
dev.off()
...
for (i in seq(1, (length(MMPs)-2), 1))
{
