...
plt.all.Mmps <- ggplot(matrix.umap.m[ grep("Mmp", matrix.umap.m$variable), ], aes(x=UMAP_1, y=UMAP_2, colour=value, group=variable)) +
                ...
                xlab("UMAP 1") + ylab("UMAP 2") + ylim(NA,10) +
...
plt.main.Mmps <- ggplot( subset(matrix.umap.m, variable == "Mmp2" | variable == "Mmp14" | variable == "Mmp25"), 
                 ...
                 geom_point(alpha=1, size=0.25) + ylim(NA,10) +
...
plt.main.Mmps.625 <- ggplot() +
                     ...
                     #scale_colour_gradient2(name="Expression", low="blue", mid="blue", high="red") +
                     scale_colour_gradientn(name="Expression", colours = c("lightgrey", "grey", "red"), values = scales::rescale(c(0, 10, 15, 33, 100)) ) +
                     xlim(-0.2,10) +  ylim(-10,10) + xlab("UMAP 1") + ylab("UMAP 2") + coord_fixed() +
                     facet_wrap( ~variable, nrow=1) +
...
pdf(paste0(Project, "_scUMAPs.Main.Mmp.EPI.6.plus", ".pdf"), width=6,height=2)
par(bg=NA)
plt.main.Mmps.625
dev.off()
...
plt.main.Mark <- ggplot( subset(matrix.umap.m, variable == "T" | variable == "Nodal" | variable == "Otx2" | variable == "Fam25c"), 
                 ...
                 axis.text.x = element_text(size=6), axis.text.y = element_text(size=6),
                 legend.key.height=unit(0.5, "lines"), legend.key.width=unit(0.3, "lines") )
pdf(paste0(Project, "_scUMAPs.Markers", ".pdf"), width=6,height=2)
par(bg=NA)
plt.main.Mark
dev.off()
...
pdf(paste0(Project, "_scUMAPs.Figure", ".pdf"), width=6,height=4)
par(bg=NA)
plot_grid(plt.main.Mmps.625, plt.main.Mark.625, ncol=1, rel_widths=c(1,1))
dev.off()
...
plt.all.Lams <- ggplot( subset(matrix.umap.m, grepl("Lam", variable)), aes(x=UMAP_1, y=UMAP_2, colour=value, group=variable)) +
  ...
  theme(aspect.ratio=1, text = element_text(size=8), 
        axis.text.x = element_text(size=6), axis.text.y = element_text(size=6),
        legend.key.height=unit(0.5, "lines"), legend.key.width=unit(0.3, "lines"))
pdf(paste0(Project, "_scUMAPs.Laminins", ".pdf"), width=6,height=2)
par(bg=NA)
plt.all.Lams
dev.off()
...
plt.all.Cols <- ggplot( subset(matrix.umap.m, grepl("Col", variable)), aes(x=UMAP_1, y=UMAP_2, colour=value, group=variable)) +
  ...
  theme(aspect.ratio=1, text = element_text(size=8), 
        axis.text.x = element_text(size=6), axis.text.y = element_text(size=6),
        legend.key.height=unit(0.5, "lines"), legend.key.width=unit(0.3, "lines"))
pdf(paste0(Project, "_scUMAPs.Collagens", ".pdf"), width=6,height=2)
par(bg=NA)
plt.all.Cols
dev.off()
